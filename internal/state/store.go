...
func EnsureParentDir(path string) error {
	...
	return os.Chmod(dir, 0o700) // #nosec G302 -- mock state directory must be owner-only searchable.
}
...
func SeedDefault(workspaceID string) *State {
	...
	ws.Flows["subscription-renewal"] = &Flow{
		...
		Steps: []FlowStep{
			...
			{ID: "branch-payment-method", Type: "function", Title: "Choose payment path",
				InputSchema:  "{paymentMethod: any}",
				OutputSchema: "{path: \"card\"|\"invoice\"}",
				Definition:   "(step :function :branch-payment-method {:code '(fn [{:keys [paymentMethod]}] (if (= \"invoice\" (get-in paymentMethod [:body :type])) {:path \"invoice\"} {:path \"card\"}))})"},
			...
			{ID: "branch-status", Type: "function", Title: "Choose success/failure path",
				InputSchema:  "{paymentStatus: any}",
				OutputSchema: "{path: \"success\"|\"failed\"}",
				Definition:   "(step :function :branch-status {:code '(fn [{:keys [paymentStatus]}] (if (= \"succeeded\" (get paymentStatus :status)) {:path \"success\"} {:path \"failed\"}))})"},
	...
	ws.Flows["send-receipt-subflow"] = &Flow{
		...
		Steps: []FlowStep{
			...
			{ID: "audit-log", Type: "function", Title: "Audit log",
				InputSchema:  "{messageId: string, email: string}",
				OutputSchema: "{ok: boolean}",
				Definition:   "(step :function :audit-log {:code '(fn [x] (assoc x :ok true))})"},
		},
	}
	...
	ws.Flows["customer-billing-healthcheck"] = &Flow{
		...
		Spine: []string{
			...
			"⫵ join / wait for all",
			"├─ function: enrich-customer         (enrich-customer)",
			"╭─ loop: for-each invoice",
			"│  ├─ function: validate-invoice     (validate-invoice)",
			"│  └─ http: mark-invoice         (mark-invoice)",
			"╰─ next item",
			"├─ ◇ branch: customer segment / overdue (branch-remediation)",
			"│  ├─ premium → function: priority-handling          (priority-handling)",
			"│  ├─ overdue → ↗ call subflow: send-reminder-subflow (call-send-reminder)",
			"│  └─ else    → function: standard-processing        (standard-processing)",
		...
		Steps: []FlowStep{
			...
			{ID: "enrich-customer", Type: "function", Title: "Enrich Customer", InputSchema: "{customer: any}", OutputSchema: "{customer: any, segment: string}", Definition: "(step :function :enrich-customer {:code '(fn [x] (assoc x :segment \"premium\"))})"},
			{ID: "validate-invoice", Type: "function", Title: "Validate Invoice", InputSchema: "{invoice: any}", OutputSchema: "{ok: boolean, overdue: boolean}", Definition: "(step :function :validate-invoice {:code '(fn [x] {:ok true :overdue false})})"},
			...
			{ID: "branch-remediation", Type: "function", Title: "Decide remediation path (premium/overdue/else)", InputSchema: "{segment: string, overdue: boolean}", OutputSchema: "{path: \"premium\"|\"overdue\"|\"else\"}", Definition: "(step :function :branch-remediation {:code '(fn [{:keys [segment overdue]}] (cond (= segment \"premium\") {:path \"premium\"} overdue {:path \"overdue\"} :else {:path \"else\"}))})"},
			{ID: "priority-handling", Type: "function", Title: "Priority Handling", InputSchema: "{customer: any}", OutputSchema: "{ok: boolean}", Definition: "(step :function :priority-handling {:code '(fn [x] {:ok true})})"},
			...
			{ID: "standard-processing", Type: "function", Title: "Standard Processing", InputSchema: "{customer: any}", OutputSchema: "{ok: boolean}", Definition: "(step :function :standard-processing {:code '(fn [x] {:ok true})})"},
	...
	ws.Flows["send-reminder-subflow"] = &Flow{
		...
		Steps: []FlowStep{
			...
			{ID: "audit", Type: "function", Title: "Audit", InputSchema: "{context: any}", OutputSchema: "{ok: boolean}", Definition: "(step :function :audit {:code '(fn [x] {:ok true})})"},
		},
	}
	...
	ws.Flows["daily-sales-report"] = &Flow{
		...
		Steps: []FlowStep{
			...
			{ID: "calculate-metrics", Type: "function", Title: "Calculate Sales Metrics",
				InputSchema:  "{sales: any[]}",
				OutputSchema: "{totalSales: number, transactionCount: number, averageOrder: number}",
				Definition:   "(step :function :calculate-metrics {:input {:sales sales} :code '(fn [input] ...)})"},
	...
	r1 := &Run{
		...
		Steps: []StepExecution{
			...
			{StepID: "calculate-metrics", StepType: "function", Title: "Calculate Sales Metrics", Status: "running", Attempt: 1,
	...
	ws.Runs["4821"] = &Run{
		...
		Steps: []StepExecution{
			...
			{StepID: "branch-payment-method", StepType: "function", Title: "Choose payment path", Status: "completed", Attempt: 1,
			...
			{StepID: "branch-status", StepType: "function", Title: "Choose success/failure path", Status: "completed", Attempt: 1,
...
func EnsureDefaults(st *State, workspaceID string) {
	...
	if ws.Profiles == nil {
		ws.Profiles = map[string]*Profile{}
	}
	if ws.Triggers == nil {
