...
- primitive-first ladder:
  ...
  - when copying overall flow structure, prefer ` + "`breyta flows templates duplicate <template-slug>`" + ` over ` + "`breyta flows grep --scope templates --full --raw-definition`" + ` copy/paste; raw-definition output is normalized inspection data, while ` + "`breyta flows pull <slug>`" + ` preserves editable workspace formatting
...
- installable checklist: explicit author approval, source visibility flags as listing intent, ` + "`breyta flows public publish <slug>`" + ` after release, wait for ` + "`published`" + ` or poll with ` + "`breyta flows public status <slug>`" + `, pushed/diffed/released/promoted live version, owner setup proof when required, Discover install dialog or installation create/configure/enable proof, installer-owned binding proof, installed run, output review
...
- treat ` + "`ok=true`" + ` responses with ` + "`meta.timedOut=true`" + ` or ` + "`data.wait.timedOut=true`" + ` as pending rather than proof of completion; inspect the workflow or rerun with a longer ` + "`--timeout`" + `
...
func ensurePublicFlowReuseDuringAuthoring(body string) string {
	readinessGuidance := strings.Join([]string{
		...
		"`breyta flows installations list <flow-slug> --all` for one flow in the current workspace.",
		"Flow creators can use `breyta flows installations list <flow-slug> --all-users` for all consumer users.",
	}, "\n")
