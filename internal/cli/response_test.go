...
func TestWriteData_PreservesMetaAddedByLinkEnrichment(t *testing.T) {
	app := &App{
		WorkspaceID: "ws-acme",
		APIURL:      "https://breyta.example.test",
	}
	out := new(bytes.Buffer)
	...
	if got, _ := meta["webUrl"].(string); got != "https://breyta.example.test/ws-acme/runs/daily-sales-report/wf-123" {
...
func TestEngineUIURLPreservesReverseProxyMountPath(t *testing.T) {
	...
	want := "https://example.test/breyta/ws-acme/runs"
	if got != want {
...
func TestNormalizeResourceWebURLUsesStructuralResourceKind(t *testing.T) {
	...
	want := "https://example.test/ws-acme/storage"
