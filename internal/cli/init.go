...
6. Build in small slices: contract -> ` + "`flows init`" + ` -> one ` + "`flows steps create/update`" + ` boundary -> ` + "`flows steps run`" + ` -> target checks -> scoped connection status -> interface run -> inspect output. When authored defaults are sufficient, add ` + "`--run`" + ` to create/update to persist and prove the step in one command; use ` + "`--run-idempotency-key`" + ` for side-effectful proof.
   - ` + "`flows init`" + ` now defaults to an empty draft with a no-input manual ` + "`run`" + ` interface. If the first packaged step is already a complete EDN literal, pass ` + "`--step-id`" + ` and ` + "`--step-file`" + `; add ` + "`--run`" + ` only when authored-default proof is intended. Add schedules explicitly with ` + "`flows schedules upsert`" + ` and validate them before release.
...
- Iterate in draft with the step-first API first: ` + "`breyta flows init`" + `, ` + "`breyta flows steps create/update/run`" + `, ` + "`breyta flows steps checks run`" + `, ` + "`breyta flows connections status <slug> --source draft --step <step-id>`" + `, interface/schedule commands, then full source lint/push only when a whole-definition edit is intended. Add ` + "`--run`" + ` to create/update for a write-plus-proof response (` + "`data.write`" + ` and ` + "`data.run`" + `); use standalone run for custom params.
- ` + "`flows init <slug>`" + ` supplies the default empty draft and no-input manual ` + "`run`" + ` interface. Seed a complete first packaged step with ` + "`--step-id`" + ` and ` + "`--step-file`" + `, and add ` + "`--run`" + ` only for an explicit proof. Use ` + "`flows schedules upsert`" + ` for cron/timezone behavior rather than inventing schedule defaults.
