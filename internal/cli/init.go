...
7. For a new flow, create the local source with ` + "`breyta flows init <slug>`" + `; repeat ` + "`--input 'name:type[:required|optional[:label]]'`" + ` to seed manual invocation inputs. When the first packaged literal is already ready, seed it with ` + "`breyta flows init <slug> --step-id <id> --step-file <path> [--run]`" + `. For an existing flow, use ` + "`breyta flows pull <slug>`" + `. Add later packaged definitions with ` + "`breyta flows steps create/update/remove`" + ` and compose only the orchestration body with ` + "`breyta flows compose`" + `.
8. Build in small slices: contract -> manual interface -> one boundary -> lint -> optional local step run -> explicit push -> configure-check -> run -> inspect output. ` + "`flows steps run`" + ` sends the complete local literal for just-in-time execution and does not persist a draft; its ` + "`--timeout`" + ` defaults to 15 minutes and is also available on init/create/update ` + "`--run`" + `. If init/create saves locally but ` + "`--push`" + ` or ` + "`--run`" + ` fails, use ` + "`meta.localPath`" + ` and ` + "`meta.nextCommands`" + ` from the error; run the reported push/update/run command instead of rerunning init/create. ` + "`breyta flows push --file ...`" + ` waits up to two minutes per draft-upload and immediate-validation API request by default; use ` + "`--timeout 5m`" + ` for slower workspaces. If it times out, check ` + "`breyta flows show <slug>`" + ` or ` + "`breyta flows validate <slug>`" + ` before retrying because the draft may already be saved.
...
- After pushing and releasing, use ` + "`breyta flows public publish <slug>`" + ` to submit a candidate to the asynchronous marketplace acceptance gate; add ` + "`--wait`" + ` or poll with ` + "`breyta flows public status <slug>`" + `. Use ` + "`breyta flows public delist <slug>`" + ` to remove it from all public surfaces immediately. Use lower-level Discover/marketplace update commands only for intentional single-flag changes; pushed visibility metadata cannot bypass review.
