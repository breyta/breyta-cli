...
7. For a new flow, create the local source with ` + "`breyta flows init <slug>`" + `; repeat ` + "`--input 'name:type:required|optional:Label'`" + ` to seed invocation fields. When the first packaged literal is already ready, seed it with ` + "`breyta flows init <slug> --step-id <id> --step-file <path> [--run]`" + `. For an existing flow, use ` + "`breyta flows pull <slug>`" + `. Add later packaged definitions with ` + "`breyta flows steps create/update/remove`" + ` and compose only the orchestration body with ` + "`breyta flows compose`" + `.
