...
8. Build in small slices: contract -> manual interface -> one boundary -> lint -> optional local step run -> explicit push -> configure-check -> run -> inspect output. ` + "`flows steps run`" + ` sends the complete local literal for just-in-time execution and does not persist a draft.
