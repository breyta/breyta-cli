...
6. Include workspace-wide installed-app inventory when it matters: ` + "`breyta flows installations list --all`" + `; for one flow in the current workspace, use ` + "`breyta flows installations list <flow-slug> --all`" + `. Flow creators can use ` + "`breyta flows installations list <flow-slug> --all-users`" + ` for the cross-consumer view.
7. For a new flow, create the local source with ` + "`breyta flows init <slug>`" + ` and edit the generated ` + "`:invocations`" + ` contract when manual inputs are required. When the first packaged literal is already ready, seed it with ` + "`breyta flows init <slug> --step-id <id> --step-file <path> [--run]`" + `. For an existing flow, use ` + "`breyta flows pull <slug>`" + `. Add later packaged definitions with ` + "`breyta flows steps create/update/remove`" + ` and compose only the orchestration body with ` + "`breyta flows compose`" + `.
8. Build in small slices: contract -> manual interface -> one boundary -> lint -> optional local step run -> explicit push -> configure-check -> run -> inspect output. ` + "`flows steps run`" + ` sends the complete local literal for just-in-time execution and does not persist a draft; its ` + "`--timeout`" + ` defaults to 15 minutes and is also available on init/create/update ` + "`--run`" + `. If init/create saves locally but ` + "`--push`" + ` or ` + "`--run`" + ` fails, use ` + "`meta.localPath`" + ` and ` + "`meta.nextCommands`" + ` from the error; run the reported push/update/run command instead of rerunning init/create. Run ` + "`breyta flows push --file ...`" + ` without optional timeout flags for compatibility across installed CLI versions. If it times out or disconnects, check ` + "`breyta flows show <slug>`" + ` or ` + "`breyta flows validate <slug>`" + ` before retrying because the draft may already be saved.
9. Keep source installable-minded: no hardcoded workspace IDs, user emails, secrets, private URLs, or author-only resource IDs.
10. Persist large or unknown payloads with ` + "`:persist`" + ` and pass resource refs, not large inline bodies. For blob persists, choose retained/default for durable or user-visible artifacts and ` + "`:tier :ephemeral`" + ` on streaming ` + "`:http`" + ` steps for temporary downloads, exports, generated media, or API response blobs that only need short-lived workflow consumption.
11. Keep functions map-oriented; prefer Clojure map access plus ` + "`json/*`" + ` and ` + "`breyta.sandbox/*`" + ` helpers over custom parser/guard layers.
...
- ` + "`breyta flows steps run`" + ` and ` + "`breyta steps run --flow`" + ` execute supplied literals or primitive probes; they are not named existing-step probes. To verify one existing step with the flow's configured bindings, use ` + "`breyta flows run-step <slug> <step-id> --target live --input '{...}' --wait`" + `.
...
- Run ` + "`breyta flows push --file ...`" + ` without optional timeout flags for compatibility across installed CLI versions. If it times out or disconnects, check ` + "`breyta flows show <slug>`" + ` or ` + "`breyta flows validate <slug>`" + ` before retrying because the draft may already be saved.
...
- Authoring reads are compact by default. Use ` + "`--full`" + ` on ` + "`flows show`" + `, ` + "`flows diff`" + `, or ` + "`runs show`" + ` only when you need source, full diff text, steps, or result payloads. Use ` + "`flows pull`" + ` for formatting-preserving editable source; ` + "`flows grep --scope templates --full --raw-definition`" + ` is normalized template inspection data and must not be pushed.
...
- After pushing and releasing, use ` + "`breyta flows public publish <slug>`" + ` to move a flow onto all public surfaces; use ` + "`breyta flows public delist <slug>`" + ` to remove it from all public surfaces. Use lower-level Discover/marketplace update commands only for intentional single-flag changes.
...
- For workspace-wide installed-app inventory, use ` + "`breyta flows installations list --all`" + `; for one flow in the current workspace, use ` + "`breyta flows installations list <flow-slug> --all`" + `. Flow creators can use ` + "`breyta flows installations list <flow-slug> --all-users`" + ` for the cross-consumer view.
