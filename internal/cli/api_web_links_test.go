...
func TestWebLinks_FlowCommandAddsWebURL(t *testing.T) {
	...
	if got, _ := meta["webUrl"].(string); got != srv.URL+"/ws-acme/flows?flow=daily-sales-report" {
		t.Fatalf("unexpected meta.webUrl: %q", got)
	}
	data, _ := out["data"].(map[string]any)
	flow, _ := data["flow"].(map[string]any)
	if got, _ := flow["webUrl"].(string); got != srv.URL+"/ws-acme/flows?flow=daily-sales-report" {
...
func TestWebLinks_RunCommandAddsRunURLs(t *testing.T) {
	...
	meta, _ := out["meta"].(map[string]any)
	if got, _ := meta["webUrl"].(string); got != srv.URL+"/ws-acme/runs?run=wf-123" {
		t.Fatalf("unexpected meta.webUrl: %q", got)
	}
	data, _ := out["data"].(map[string]any)
	run, _ := data["run"].(map[string]any)
	if got, _ := run["webUrl"].(string); got != srv.URL+"/ws-acme/runs?run=wf-123" {
		t.Fatalf("unexpected run.webUrl: %q", got)
	}
	if got, _ := run["outputWebUrl"].(string); got != srv.URL+"/ws-acme/runs?run=wf-123" {
...
func TestWebLinks_RunCommandReplacesHostedTopLevelWebURL(t *testing.T) {
	...
	meta, _ := out["meta"].(map[string]any)
	want := srv.URL + "/ws-acme/runs?run=wf-123"
	if got, _ := meta["webUrl"].(string); got != want {
		t.Fatalf("unexpected meta.webUrl: got %q want %q", got, want)
	}
}

func TestWebLinks_RunWithoutFlowSlugGetsCanonicalURLs(t *testing.T) {
	srv := newLocalTestServer(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(map[string]any{
			"ok": true,
			"data": map[string]any{
...
func TestWebLinks_ResourcesListBuildsPortableRunURLForFlowOutputItemWithoutWebURL(t *testing.T) {
	...
	if got, _ := first["webUrl"].(string); got != srv.URL+"/ws-acme/runs?run=wf-123" {
...
func TestWebLinks_ConnectionRESTAddsWebURL(t *testing.T) {
	...
	meta, _ := out["meta"].(map[string]any)
	if got, _ := meta["webUrl"].(string); got != srv.URL+"/ws-acme/connections?connection=conn-123" {
		t.Fatalf("unexpected meta.webUrl: %q", got)
	}
	data, _ := out["data"].(map[string]any)
	if got, _ := data["webUrl"].(string); got != srv.URL+"/ws-acme/connections?connection=conn-123" {
		t.Fatalf("unexpected data.webUrl: %q", got)
	}
}
...
func TestWebLinks_ResourcesGetAbsolutizesRelativeWebURL(t *testing.T) {
	...
	data, _ := out["data"].(map[string]any)
	if got, _ := data["webUrl"].(string); got != srv.URL+"/ws-acme/runs?run=wf-123" {
		t.Fatalf("unexpected data.webUrl: %q", got)
	}
}
