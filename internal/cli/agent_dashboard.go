...
func newAgentDashboardApplyCmd(app *App) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if !cmd.Flags().Changed("expected-revision") {
				return writeErr(cmd, errors.New("missing --expected-revision"))
			}
			if expectedRevision < 0 {
				return writeErr(cmd, errors.New("--expected-revision must be zero or a positive integer"))
			}
	...
	cmd.Flags().IntVar(&expectedRevision, "expected-revision", 0, "Current dashboard revision, or 0 to create the first Marketing Hub")
