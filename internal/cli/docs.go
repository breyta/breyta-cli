...
func renderDocsIndexMD(root *cobra.Command, devMode bool) string {
	...
	b.WriteString("If a flow declares `:requires` slots (e.g. `:http-api` with `:auth`/`:oauth`, or `:llm-provider`), you must apply bindings once to create a profile, then enable it.\n")
	b.WriteString("Slot names must be non-namespaced keywords (e.g., `:api`, not `:ns/api`).\n\n")
	...
	b.WriteString("Activation validates required bindings and activation inputs; missing slots are reported in the error details.\n\n")
