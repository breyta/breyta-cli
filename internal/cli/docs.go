...
func renderDocsIndexMD(root *cobra.Command) string {
        ...
        b.WriteString("If a flow declares `:requires` slots (e.g. `:http-api` with `:auth`/`:oauth`, or `:llm-provider`), you must activate it once to provide credentials and create an instance/bindings.\n\n")
