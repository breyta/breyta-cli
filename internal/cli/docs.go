...
func renderDocsIndexMD(root *cobra.Command, devMode bool) string {
	...
	b.WriteString("- Push draft first: `breyta flows push --file ./tmp/flows/<slug>.clj`\n")
	b.WriteString("- Validate draft: `breyta flows validate <slug>` (and optionally `breyta flows compile <slug>`)\n")
	b.WriteString("- Confirm draft exists: `breyta flows show <slug> --source draft`\n")
	...
	b.WriteString("Default to reusing existing workspace connections when binding new slots: `breyta connections list` (or `--type llm-provider`) and set `slot.conn=conn-...`.\n\n")
	...
	b.WriteString("- `breyta flows bindings apply <slug> --set api.headers.apikey=... --set api.headers.x-api-key=...`\n")
	...
	b.WriteString("- Do not run bindings template commands after a failed `flows push`; fix/push/validate first.\n")
	b.WriteString("- If `flows show <slug> --source draft` returns `Flow not found`, create or push the flow, then retry `flows show`.\n\n")
	...
	b.WriteString("                  :headers {\"x-api-key\" \"<redacted>\"}}}\n\n")
